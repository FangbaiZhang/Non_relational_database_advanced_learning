...
- show collections 显示当前数据库下的集合（或者使用show tables,新版本增加了改命令）
- 注意：show tables 本来是mysql关系型数据可显示的是在当前数据库（mongodb中的databases）下有哪些数据表（数据表就相当于mongodb中的集合）
...
- 插入insert：
    ...
    - 插入的文档都是键值格式(类似字典)，实际是json格式(键可以不用双引号)
